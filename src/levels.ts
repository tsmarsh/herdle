...
export const LEVELS: LevelConfig[] = [
    // Level 1: Stackelberg — leader-follower, sheep react to where dog IS
    {
        ...
        setupEnvironment: (w, h) => ({
            ...
            obstacles: [],
            sheepSpawns: Array.from({length: 5}, () =>
                new Vector(w * 0.3 + (Math.random() - 0.5) * 60, h * 0.5 + (Math.random() - 0.5) * 60))
        })
    },
    ...
    // Level 2: Best Response — only one approach angle pushes sheep toward pen
    {
        ...
        setupEnvironment: (w, h) => ({
            ...
            sheepSpawns: Array.from({length: 7}, () =>
                new Vector(w * 0.45 + (Math.random() - 0.5) * 80, h * 0.2 + (Math.random() - 0.5) * 80))
        })
    },
    ...
    // Level 3: Coordination Game — flanking from two sides dominates one-side pushing
    {
        ...
        setupEnvironment: (w, h) => ({
            ...
            sheepSpawns: Array.from({length: 10}, () =>
                new Vector(w * 0.3 + (Math.random() - 0.5) * 80, h * 0.5 + (Math.random() - 0.5) * 160))
        })
    },
    ...
    // Level 4: Subgame Perfect — two separated groups, backward induction for order
    {
        ...
        setupEnvironment: (w, h) => {
            ...
            return {
                pen: new Pen(w - 250, h / 2 - 100, 200, 200),
                obstacles: obs,
                sheepSpawns: Array.from({length: 13}, (_, i) =>
                    i < 8
    ...
    // Level 5: Prisoner's Dilemma — herding one group disrupts the other; cooperate by containing
    {
        ...
        setupEnvironment: (w, h) => {
            ...
            return {
                pen: new Pen(w - 250, h / 2 - 100, 200, 200),
                obstacles: obs,
                sheepSpawns: Array.from({length: 14}, (_, i) =>
                    i < 7
    ...
    // Level 6: Dominant Strategy — three corridors, only the middle one works
    {
        ...
        setupEnvironment: (w, h) => {
            ...
            return {
                ...
                obstacles: obs,
                sheepSpawns: Array.from({length: 12}, () =>
                    new Vector(w * 0.15 + (Math.random() - 0.5) * 60, h * 0.45 + (Math.random() - 0.5) * 60))
            };
        }
    },
    ...
    // Level 7: Mechanism Design — V-funnel with gaps, dogs plug the gaps
    {
        ...
        setupEnvironment: (w, h) => {
            ...
            for (let i = 0; i <= steps; i++) {
                ...
                if (Math.abs(t - 0.4) < 0.08) continue;
            ...
            return {
                ...
                obstacles: obs,
                sheepSpawns: Array.from({length: 15}, () =>
                    new Vector(
                        w * 0.15 + Math.random() * (w * 0.2),
    ...
    // Level 8: Information Cascade — nudge critical mass, cohesion cascades through flock
    {
        ...
        setupEnvironment: (w, h) => ({
            ...
            sheepSpawns: Array.from({length: 25}, () =>
                new Vector(
    ...
    // Level 10: Nash Equilibrium — 4 dogs must hold stable positions at gaps
    {
        ...
        setupEnvironment: (w, h) => {
            ...
            for (let i = 0; i <= 8; i++) {
                const t = i / 8;
                if (Math.abs(t - 0.5) < 0.1) continue; // gap
            ...
            for (let y = h * 0.25; y <= h * 0.75; y += 50) {
                if (Math.abs(y - h * 0.4) < 30 || Math.abs(y - h * 0.6) < 30) continue; // two gaps
                obs.push(new Obstacle(w * 0.75, y, 25));
            }
            return {
                pen: new Pen(w * 0.8 - 100, h / 2 - 100, 200, 200),
                obstacles: obs,
                sheepSpawns: Array.from({length: 20}, () =>
                    new Vector(
                        w * 0.2 + Math.random() * (w * 0.3),
                        h * 0.3 + Math.random() * (h * 0.4)
                    ))
            };
        }
    ...
    // Level 11: The Precipice — single cliff edge, learn not to spook sheep toward it
    {
        ...
        setupEnvironment: (w, h) => ({
            ...
            sheepSpawns: Array.from({length: 8}, () =>
                new Vector(w * 0.3 + (Math.random() - 0.5) * 120, h * 0.5 + (Math.random() - 0.5) * 120)),
            cliffs: [
    ...
    // Level 12: Narrow Ridge — cliffs on two sides forming a corridor
    {
        ...
        setupEnvironment: (w, h) => ({
            ...
            sheepSpawns: Array.from({length: 10}, () =>
                new Vector(w * 0.2 + (Math.random() - 0.5) * 80, h * 0.5 + (Math.random() - 0.5) * 80)),
            cliffs: [
    ...
    // Level 13: The Switchback — zigzag path with cliffs on outer edges
    {
        ...
        setupEnvironment: (w, h) => ({
            ...
            sheepSpawns: Array.from({length: 12}, () =>
                new Vector(w * 0.15 + (Math.random() - 0.5) * 60, h * 0.5 + (Math.random() - 0.5) * 100)),
            cliffs: [
                // Top-left cliff
    ...
    // Level 14: Island Pen — pen on plateau surrounded by cliffs, one safe approach
    {
        ...
        setupEnvironment: (w, h) => {
            ...
            return {
                ...
                sheepSpawns: Array.from({length: 15}, () =>
                    new Vector(w * 0.2 + (Math.random() - 0.5) * 100, h * 0.5 + (Math.random() - 0.5) * 200)),
                cliffs: [
    ...
    // Level 15: Cliffhanger — multiple cliff edges creating islands with bridges
    {
        ...
        setupEnvironment: (w, h) => ({
            ...
            sheepSpawns: Array.from({length: 20}, () =>
                new Vector(w * 0.15 + (Math.random() - 0.5) * 80, h * 0.5 + (Math.random() - 0.5) * 200)),
            cliffs: [
