...
function parseTrack(view: DataView, start: number, end: number): TrackEvent[] {
    ...
    while (offset < end) {
        ...
        if (status === 0xFF) {
            ...
            offset = o2;
            const data: number[] = [];
            for (let i = 0; i < len; i++) data.push(view.getUint8(offset++));
...
function parseMidi(buffer: ArrayBuffer): { notes: NoteEvent[]; duration: number } {
    ...
    const tickTime = new Map<number, number>();
    ...
    const active = new Map<string, { time: number; velocity: number; channel: number; note: number }>();
...
export default class MusicPlayer {
    ...
    private schedulePlayback(): void {
        ...
        for (const n of this.notes) {
            if (n.channel === 9) continue; // skip percussion channel
        ...
        sentinel.onended = () => {
            if (this._playing) this.schedulePlayback();
        };
    }
}
