...
export default class Renderer {
    ...
    getObstacleFaces(obs) {
        ...
        for (let i = 0; i < sides; i++) {
            ...
            vertices.push(Math.cos(angle) * obs.radius * jitter, Math.sin(angle) * obs.radius * jitter);
        }
        faces = [];
    ...
    draw(dogs, sheep, obstacles, pen, score, total, levelName, levelComplete) {
        this.init();
        this.clear();
    ...
    drawScore(score, total, levelName) {
        this.ctx.save();
    ...
    drawPen(pen) {
        // Soft inner glow
        const innerGlow = this.ctx.createRadialGradient(pen.x + pen.width / 2, pen.y + pen.height / 2, 0, pen.x + pen.width / 2, pen.y + pen.height / 2, pen.width * 0.6);
    ...
    drawPolygon(cx, cy, radius, sides, rotation = 0) {
        this.ctx.beginPath();
        for (let i = 0; i < sides; i++) {
