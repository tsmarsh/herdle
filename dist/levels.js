...
export const LEVELS = [
    ...
    },
    {
        ...
        setupEnvironment: (w, h) => {
            const obs = [];
            for (let i = w * 0.2; i < w * 0.8; i += 60)
                obs.push(new Obstacle(i, h * 0.4, 35));
            for (let i = h * 0.4; i < h * 0.8; i += 60)
                obs.push(new Obstacle(w * 0.8, i, 35));
            for (let i = w * 0.4; i < w * 0.8; i += 60)
                obs.push(new Obstacle(i, h * 0.8, 35));
            return {
    ...
    },
    {
        ...
        setupEnvironment: (w, h) => {
            const obs = [];
            for (let i = 0; i < h * 0.7; i += 50)
                obs.push(new Obstacle(w * 0.33, i, 30));
            for (let i = h; i > h * 0.3; i -= 50)
                obs.push(new Obstacle(w * 0.66, i, 30));
            return {
    ...
    },
    {
        ...
        setupEnvironment: (w, h) => {
            ...
            for (let i = 100; i < w - 100; i += 60) {
                obs.push(new Obstacle(i, 100, 35));
                obs.push(new Obstacle(i, h - 100, 35));
            }
            for (let i = 100; i < h - 100; i += 60) {
                obs.push(new Obstacle(100, i, 35));
                if (i < h / 2 - 100 || i > h / 2 + 100)
                    obs.push(new Obstacle(w - 100, i, 35));
            }
            // Inner barrier
            for (let i = 250; i < h - 250; i += 60)
                obs.push(new Obstacle(w / 2, i, 40));
            return {
